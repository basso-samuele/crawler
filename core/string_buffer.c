...
static bool relocate(CrawlerString* string, size_t required) {
    ...
    if (string->capacity > SIZE_MAX - increment)
        return false;
    ...
    if (new_capacity > SIZE_MAX / sizeof *string->data)
        return false;
...
bool crawler_string_create(CrawlerString* string, size_t capacity) {
    ...
    if (string->data != NULL)
        return false;
    // Overflow check.
    if (capacity > SIZE_MAX / sizeof *string->data)
        return false;
...
bool crawler_string_clone(CrawlerString* destination, const CrawlerString* source) {
    ...
    if (destination->data != NULL || source->data == NULL)
        return false;
    // Overflow check.
    if (source->capacity > SIZE_MAX / sizeof *destination->data)
        return false;
...
bool crawler_string_append(CrawlerString* string, int cp) {
    ...
    if (!(string->length < string->capacity)) {
        if (!relocate(string, string->length+1))
            return false;
    }
...
bool crawler_string_append_string_buffer(CrawlerString* destination, const CrawlerString* source) {
    ...
    if (destination->capacity - destination->length < source->length) {
        ...
        if (source->length > SIZE_MAX - destination->length)
            return false;
        if (!relocate(destination, required))
            return false;
    }
